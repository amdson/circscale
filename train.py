...
class RunConfig:
    ...
    # out over passes).
    data_order: str = "iid"
    # task="concat": a small circuit (small_wires x small_depth, small_seed;
    # output small_wire only) stacked beside the rich brickwork circuit
    # (n_wires x circ_depth). Output 0 is the small target, outputs 1.. the
    # rich taps. train_frac/pool_seed/data_order apply to the small inputs
    # only; rich inputs are always fresh. small_weight: None = plain mean
    # over all outputs, else loss = w * small + (1 - w) * mean(rich).
    small_wires: int = 10
    small_depth: int = 4
    small_seed: int = 0
    small_wire: int = 0
    ...
    def name(self) -> str:
        ...
        if self.task != "brickwork":
            s += f"_{self.task}"
        if self.task == "concat":
            s += f"{self.small_wires}x{self.small_depth}s{self.small_seed}o{self.small_wire}"
            if self.small_weight is not None:
                s += f"_sw{self.small_weight:g}"
...
def run(cfg: RunConfig, stop_after: int | None = None, quiet: bool = False):
    ...
    if cfg.task == "tree3":
        ...
        n_out = cfg.n_wires
    elif cfg.task == "concat":
        rich = sample_circuit(crng, cfg.n_wires, cfg.circ_depth)
        small = sample_circuit(np.random.default_rng(cfg.small_seed),
                               cfg.small_wires, cfg.small_depth)
        rich_eval, small_eval = make_jax_evaluator(rich), make_jax_evaluator(small)
        n_sm, sm_out = cfg.small_wires, cfg.small_wire

        def circ_eval(x):
            return jnp.concatenate(
                [small_eval(x[:, :n_sm])[:, sm_out:sm_out + 1], rich_eval(x[:, n_sm:])], axis=1)

        class circuit:  # only out_depths is used below
            out_depths = np.concatenate([[small.out_depths[sm_out]], rich.out_depths])
        n_out = 1 + cfg.n_wires
    else:
        raise ValueError(f"unknown task {cfg.task!r}")
    concat = cfg.task == "concat"
    if concat and cfg.train_frac is None:
        raise ValueError("task='concat' requires train_frac (the small-circuit split)")
    n_in = cfg.n_wires + (cfg.small_wires if concat else 0)
    pool_wires = cfg.small_wires if concat else cfg.n_wires  # enumerated by train_frac
    mlp_cfg = MLPConfig(
    ...
    if cfg.train_frac is not None:
        ...
        n_all = 2 ** pool_wires
        all_x = ((np.arange(n_all)[:, None] >> np.arange(pool_wires)[::-1]) & 1
                 ).astype(np.uint8)
    ...
    def train_step(params, opt_state, step):
        ...
        if concat:  # small inputs from the pool, rich inputs always fresh
            x_rich = jax.random.bernoulli(jax.random.fold_in(key, 1),
                                          shape=(cfg.batch, cfg.n_wires))
            x = jnp.concatenate([x, x_rich.astype(jnp.uint8)], axis=1)
        y = circ_eval(x).astype(jnp.float32)
        def loss_fn(p):
            pol = per_output_bce(forward(p, x), y)
            if concat and cfg.small_weight is not None:
                return cfg.small_weight * pol[0] + (1 - cfg.small_weight) * jnp.mean(pol[1:])
    ...
    if not pooled:
        ...
        if concat:  # pair each small input with fresh rich inputs
            eval_x = jnp.concatenate([eval_x, jax.random.bernoulli(
                jax.random.key(cfg.eval_seed), shape=(n_all, cfg.n_wires)
            ).astype(jnp.uint8)], axis=1)
